...
class Potential:
    ...
    def orbit_integrator_run_notdense(self,w0,t0,t1,ts, steps):
        ...
        max_steps: int = 50_000#16**4#8100#4000
        t0 = t0#0.0
        t1 = t1#4000.
        dense = True# WAS TRUE MADE FALSE.. NEED TRUE FOR GRADIENTS!!!! MAXSTEPS!
        #y0= w_init
    ...
    @partial(jax.jit,static_argnums=((0,5)))
    def orbit_integrator_run_notdense_PID(self,w0,t0,t1,ts, steps):
        term = ODETerm(self.velocity_acceleration)
        solver = diffrax.Dopri5()#(scan_kind="bounded") #Dopri5, Dopri8
        saveat = SaveAt(t0=False, t1=True,ts=ts,dense=False)
        ...
        #    saveat = SaveAt(t0=False, t1=True,ts=ts,dense=False) # ts=ts,
        rtol: float = 1e-6#1e-6  
        atol: float = 1e-6  #1e-6  
        def true_func(dt):
            return .5#.5
        def false_func(dt):
            return -.5#-.5
        dt0= None#jax.lax.cond(t0<t1,true_func,false_func,None) #None
        
       
        stepsize_controller = PIDController(rtol=rtol, atol=atol, dtmin=.5,force_dtmin=True,
                                            )#.5 dtmin = 0.05#ConstantStepSize()#PIDController(rtol=rtol, atol=atol, dtmin=0.05,force_dtmin=True)#ConstantStepSize()#PIDController(rtol=rtol, atol=atol, dtmin=0.005,force_dtmin=True)#ConstantStepSize()# ## #dtmin 0.01
    ...
    def orbit_integrator_run_and_deriv_noPreComp_mass_radius(self,w0,t0,t1, ts, dense_arg):
        ...
        rtol: float = 1e-18#1e-18#1e-11 1e-9#1e-9
        atol: float = 1e-18#1e-18#1e-11 1e-9#1e-9 
        dt0=None
        stepsize_controller = PIDController(rtol=rtol, atol=atol, dtmin=.008,force_dtmin=True) # dtmin = 0.05, dtmax=100
        max_steps: int = 50_000#4_000#50_000#16**4#16**4 #was 16**5
        t0 = t0
        t1 = t1
        dense = False
        terminating_event =  None#DiscreteTerminatingEvent(event_fun) #None
    ...
    def orbit_integrator_run_StreamSpline_PID_mass_radius_SingleStar(self,w0,t0,t1, ts, state):
        ...
        solver = diffrax.Dopri5(scan_kind="bounded") #diffrax.Dopri5(scan_kind="bounded")#Heun(scan_kind="bounded")
        saveat = SaveAt(t0=False, t1=True, ts=ts, dense=False,steps=False)###CHANGE DENSE TO FALSE AGAIN!
        rtol: float = 1e-13# 1e-15 dtmin 0.2 | 1e-13 with Dopri5 also worked
        atol: float = 1e-13# 1e-15 dtmin 0.2 | 1e-13 with Dopri5 also worked
        dt0=None
    ...
    ### SELF GRAV
    @partial(jax.jit,static_argnums=(0,))
    def velocity_acceleration_perturbation_from_PreComp_mass_radius_SingleStar_selfGrav(self,t,xv,args):
        """
        
        """
        ind, stream_func, prog_func = args
        w0_at_t = interp_func(t,ind,stream_func)

        prog_derivs = prog_func.evaluate(t)[1] # nsh x 12
        ...
        # tidal tensor of progenitor
        tidal_prog = -jax.jacrev((self.gradientPotentialProgenitor))(x0,t) # 3 x 3
        du_deps = x1 - prog_derivs[:,:3] # nsh x 3
        self_grav_term = jnp.einsum('ij,kj->ki',tidal_prog,du_deps,optimize='optimal') # nsh x 3
        ...
        d_qdot_d_eps = v1 
        # this is dpdot/deps. Injected acceleration + matmul(tidal tensor, q)
        d_pdot_d_eps = acceleration1 + self_grav_term + jnp.einsum('ij,kj->ki',d2H_dq2,x1,optimize='optimal') #nSH x 3 #jnp.matmul(d2H_dq2,x1) #jnp.einsum('ijk,ij->ik',d2H_dq2,x1,precision=jax.lax.Precision('highest'))   #n_star x 3
        
        # Now handle radius deviations
        du_deps_drs = dx1_dtheta - prog_derivs[:,3:] # nsh x 3
        self_grav_term_r = jnp.einsum('ij,kj->ki',tidal_prog,du_deps_drs,optimize='optimal') # nsh x 3
        acceleration1_r = -self.gradientPotentialStructural_per_SH(x0,t) # nSH x 3
        d_qalpha1dot_dtheta = dv1_dtheta
        d_palpha1dot_dtheta = acceleration1_r + self_grav_term_r + jnp.einsum('ij,kj->ki',d2H_dq2,dx1_dtheta, optimize='optimal')#jnp.matmul(d2H_dq2,dx1_dtheta)
    ...
    @partial(jax.jit,static_argnums=(0,))
    def orbit_integrator_run_StreamSpline_PID_mass_radius_SingleStar_selfGrav(self,w0,t0,t1, ts, state):
        #def event_fun(state, **kwargs):
        #    return state.tnext > 0.0#>=0.02
        #spl, ind = state
        #arr, narr = eqx.partition(spl, eqx.is_array)
        ...
        term = ODETerm(self.velocity_acceleration_perturbation_from_PreComp_mass_radius_SingleStar_selfGrav)
        solver = diffrax.Dopri5(scan_kind="bounded") #diffrax.Dopri5(scan_kind="bounded")#Heun(scan_kind="bounded")
        saveat = SaveAt(t0=False, t1=True, ts=ts, dense=False,steps=False)###CHANGE DENSE TO FALSE AGAIN!
        rtol: float = 1e-13# 1e-15 dtmin 0.2 | 1e-13 with Dopri5 also worked
        atol: float = 1e-13# 1e-15 dtmin 0.2 | 1e-13 with Dopri5 also worked
...
class GeneralNFWPotential(Potential):
    """
    Follow Koposov S5 analysis Eq 7: https://arxiv.org/pdf/2211.04495
    And https://arxiv.org/pdf/astro-ph/0007354
    Essentially the same as Agama's spheroid potential
    Though the interpolation scheme makes it such that rcut is always 50 x the scale radius,
    with a cutoff strength of 2.
    """
    def __init__(self, r_s, rho0, gamma, beta, 
                 interp_func = jnp.load('general_nfw_interp.npy',allow_pickle=True).item(),
                 units=None):
        super().__init__(units, {'r_s': r_s, 'rho0': rho0, 'gamma':gamma,
                                'beta':beta, 'interp_func':interp_func })
        ##self.interp_func = jnp.load('general_nfw_interp.npy',allow_pickle=True).item()
    
    ##@jax.jit
    def potential(self, xyz, t):
        r = jnp.sqrt(jnp.sum(xyz**2)) 
        r_over_r_s = r/self.r_s
        bracket = self.interp_func(r_over_r_s,self.gamma,self.beta)*(self.r_s**2)
        return -4*jnp.pi*self._G*bracket*self.rho0
    
class EinastoPotential(Potential):
    """
    Implentation from https://www.aanda.org/articles/aa/pdf/2012/04/aa18543-11.pdf
    """
    def __init__(self, r_s, n, M, units=None):
        super().__init__(units, {'r_s': r_s, 'n': n, 'M':M})
        self.d_n = 3*self.n - (1/3) + (8/(1215*self.n)) + (184/(229635*(self.n**2))) + (1084/(31000725.0*(self.n**3))) - (17557576/(1242974068875*(self.n**4)))
        
    @partial(jax.jit,static_argnums=(0,))
    def gamma_inc_func(self, x=None,a=None):
        return gamma_func(a)*gammaincc(x=x,a=a)
    ...
    def potential(self, xyz, t):
        ...
        r = jnp.sqrt(jnp.sum(xyz**2)) 
        d_n_val = self.d_n
        d_n_val_to_the_n = (d_n_val**self.n)
        h = self.r_s / d_n_val_to_the_n
        s = d_n_val_to_the_n*r / self.r_s
        s_to_the_1_n = s**(1./self.n)
        prefac = (self._G*self.M/h)*(s**(-1))
        gamma_eval_3n = gamma_func(3*self.n)
        
        
        return -prefac*(1.0 - (self.gamma_inc_func(a=3*self.n, x=s_to_the_1_n)/gamma_eval_3n) + s*( self.gamma_inc_func(a=2*self.n,x=s_to_the_1_n) / gamma_eval_3n )  )
    
class DehnenPotential(Potential):
    def __init__(self, gamma, M, r_s, units=None):
        super().__init__(units, {'gamma': gamma, 'M': M, 'r_s':r_s})
        self.pred = (self.gamma == 2.0)
        
    @partial(jax.jit,static_argnums=(0,))
    def false_func(self, r):
        prefac = self._G*self.M/self.r_s
        pot_out = -(1./(2-self.gamma+.00000000000001))*(1. - (r / (r+self.r_s))**(2-self.gamma))
        return prefac*pot_out
    @partial(jax.jit,static_argnums=(0,))
    def true_func(self, r):
        prefac = self._G*self.M/self.r_s
        pot_out = jnp.log(r / (r + self.r_s))
        return prefac*pot_out
        
    @partial(jax.jit,static_argnums=(0,))
    def potential(self, xyz, t):
        r = jnp.sqrt(jnp.sum(xyz**2))
        return jax.lax.cond(self.pred, self.true_func, self.false_func, r)
...
class PotentialBase_and_PotentialPerturbation_and_PotentialStructural_andSelfGrav(Potential):
    def __init__(self, potential_base, potential_perturbation, potential_structural, PotentialProgenitor, units=None):
        super().__init__(units,{'potential_base':potential_base, 
                                'potential_perturbation':potential_perturbation, 
                                'potential_structural':potential_structural,
                                'PotentialProgenitor':PotentialProgenitor})
        self.gradient = None
        self.gradientPotentialBase = potential_base.gradient
        self.gradientPotentialPerturbation = potential_perturbation.gradient
        self.gradientPotentialStructural = potential_structural.gradient
        self.gradientPotentialProgenitor = PotentialProgenitor.gradient
